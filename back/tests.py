...
class VcapTestCase(unittest.TestCase):
    def setUp(self):
        self.VCAP_SERVICES  = { 'sendgrid': [{
            'name': 'waasapi-email',
            'instance_name': 'waasapi-email',
            'credentials': {
    ...
    def test_parsing(self):
        credentials = self.VCAP_SERVICES['sendgrid'][0]['credentials']
        self.assertEqual(credentials['password'], '__password__')
        self.assertEqual(credentials['username'], '__username__')
        self.assertEqual(credentials['hostname'], '__hostname__')
